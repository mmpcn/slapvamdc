...
\textit{experiment},  \textit{theory}, or \textit{ritz} \\
\hline
...
\ref{sec:Protocol}.\todo{explain about convertion function?}
...
TBD\todo{We don't have any well-known lines in TOSS; perhaps publish
splatalogue in linetap?}
...
located using SQL patterns.  The example from the use case sould
translate into

% please-run-a-test-lenient
\begin{lstlisting}[language=SQL]
SELECT *
FROM toss.line_tap
WHERE
  inchi LIKE 'InChI=1S/C16H10/%'
\end{lstlisting}

For a well-defined species, clients should use the InChi key.  For
normal Methane, that would be:
...
-- for singly deuterated Methane, the InChI key would be
...
\item [vacuum\_wavelength (in J)]\todo{ in J? According to VAMDC docs, RadTransWavelength returns values in Angstrom. So it needs be converted twice: to J, and from J to the users preferred unit}
  \begin{xsamsbridge}
...
\item [upper\_state\_configuration]
  \begin{xsamsbridge}
    ...
    \item[returnables]
      \xsref{AtomStateDescription}\footnote{\url{https://standards.vamdc.eu/dictionary/returnables.html\#atomstatedescription}},
      \xsref{MoleculeStateDescription}\footnote{\url{https://standards.vamdc.eu/dictionary/returnables.html\#moleculestatedescription}}
...
\item [upper\_state\_energy]
  \begin{xsamsbridge}
    \item[data model]
     ...
     \item[returnables]
     \xsref{AtomicStateEnergy},\footnote{\url{https://standards.vamdc.eu/dictionary/returnables.html\#atomstateenergy}}
      \xsref{MolecularStateEnergy},\footnote{\url{https://standards.vamdc.eu/dictionary/returnables.html\#atomstateenergy}}
      \xsref{RadTransUpperStateRef},
      \xsref{AtomStateID},
      \xsref{MoleculeStateID}
    \item[constraints]
...
\item [title]
      a human readable string composed by the values of \texttt{stoichiometric\_formula} and
     \texttt{ion\_charge}. This quantity is not part of XSAMS (but part of SSLDM).\todo{Done. Not in XSAMS?What about Molecules?}
